...
type System struct {
	Cfg       config.Config
	Pool      *pgxpool.Pool
	ReadOnly  *pgxpool.Pool // may equal Pool in dev when no separate DSN is set
...
func Build(ctx context.Context, cfg config.Config) (*System, error) {
	...
	// Read-only SQL runs on a dedicated pool (defense in depth). In dev,
	// without IG_READONLY_DATABASE_URL, the primary DSN is reused but every
	// query still executes inside a READ ONLY transaction with timeouts.
	readOnlyURL := cfg.ReadonlyDatabaseURL
	if readOnlyURL == "" {
		readOnlyURL = cfg.DatabaseURL
	}
	roPool, err := db.Connect(ctx, readOnlyURL)
	if err != nil {
		return nil, err
	}
	...
	if cfg.LLMProvider == "openai" && cfg.LLMAPIKey != "" {
		oai := retrieval.NewOpenAIEmbedder(cfg.LLMBaseURL, cfg.LLMAPIKey, cfg.EmbeddingModel, cfg.EmbeddingDim)
		if cfg.EmbeddingFallback == "hash" {
			oai.EnableExplicitHashFallback()
			log.Warn("embedding fallback ENABLED: real-embedding failures degrade to hash vectors",
				observability.F{})
		}
		emb = oai
	} else {
	...
	registry := tools.NewRegistry(
		...
		tools.NewQueryPostgres(roPool, ret, func(sqlText string) error {
	...
	if cfg.DurableMCPURL != "" && cfg.DurableMCPTimeout > 0 {
	...
	nativeRunner := agent.NewNative(agent.Deps{
		...
		LeaseOwner: "", // per-process identity generated by the runner
		LeaseTTL:   cfg.RunLeaseTTL,
		Log:        observability.New("native-runner"),
	})

	return &System{
		Cfg: cfg, Pool: pool, ReadOnly: roPool, Log: log,
